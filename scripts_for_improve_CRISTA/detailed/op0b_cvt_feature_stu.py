...
for i, stuid in enumerate(stulst):
    sg, neg = studt[stuid]
    df = pd.read_csv(f'ori_data/study/{neg}.csv', index_col=None)
    col = df.columns
    df = df[col[1:-1]]
    df[col[5]] = list(map(lambda s: s.replace('-', ''), df[col[5]]))
    df[col[6]] = list(map(lambda s: s.replace('-', ''), df[col[6]]))
    ID = f'({i+1} / {len(stulst)}) {stuid}'
    N = len(df)
    SG = sg
    df1 = pd.DataFrame(list(map(row_proc, filter(row_filt, df.iterrows()))))
    df1.to_csv(f'data/study/{stuid}_stu.csv', sep='\t', index=None)
